...
class RobotCommand(object):
    ...
    def __init__(self, server_addrs, client_addrs):
        ...
        self.callback = None
        self._prog_feedback_script = []
        self._prog_input_script = []
    ...
    def setFeedback(self, host, port, callback=None):
        ...
        self._prog_feedback_script = self._loadFeedbackScript(host, port)

    def setInput(self, host, port):
        self._prog_input_script = self._loadInputScript(host, port)

    def send(self, data):
        try:
            self.robot_server.socket.send(data.encode())
        except Exception as e:
            print(e)
    ...
    def load(self, proglist):
        ...
        try:
            proglist = self._prog_feedback_script + self._prog_feedback_script + proglist + ["join thrd_feedback"] + ["join thrd_input"]
            self.robot_server.sendProgram(proglist)
        except:
            print("Syntax Error")
    ...
    def _loadInputScript(self, host, port):
        _prog = [
            ...
            # 'thread inputThread():',
            # 'sock0flg = False',
            # 'while sock0flg == False:',
            # 'sock0flg = socket_open(\"{}\",{},"socket_0")'.format(host, port),
            # 'end',  # end thread
            # TODO
    ...
    def _loadFeedbackScript(self, host, port):
        _prog = [

            'global feedback_data = 0',
            'global trigger_feedback = False',
            ...
            'if trigger_feedback:',
            # send
            'socket_send_string(feedback_data,"socket_0")',  # TODO: int, string, byte
            ...
            ########################################
            "thrd_feedback = run feedbackThread()"
        ]
        return _prog
